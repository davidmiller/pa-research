...
scripclean <- function(interesting=NULL){
  ...
  write.csv(spend.practice,"spend_practice.inh.csv",row.names=FALSE,quote=FALSE)
  write.csv(problem.drugs,"problem_drugs.inh.csv",row.names=FALSE,quote=FALSE)
  write.csv(problem.spend,"problem_spend.inh.csv",row.names=FALSE,quote=FALSE)
  write.csv(total.problem.spend,"total_problem_spend.inh.csv",row.names=FALSE,quote=FALSE)

}


clean.codes <- function(codes, file.list, filesep){
  ## Set up data frames for results
  total.problem.spend<-data.frame(matrix(nrow=0,ncol=4))
  spend.practice<-data.frame(matrix(nrow=0,ncol=11))
  spend.practice.total<-data.frame(matrix(nrow=0,ncol=4))
  ...
  for (i in 1:length(file.list)){
    ...
    surgery.subtotal<-aggregate(GP.drugs[,c("ACT.COST","ITEMS")],
                                by=list(GP.drugs$PRACTICE,GP.drugs$PERIOD),FUN=sum)
    names(surgery.subtotal)<-c("Practice.code","Month","cost.alldrugs","items.alldrugs")
    t<-subset(GP.drugs,BNF.CODE %in% codes$bnf.code)
    problem.spend<-aggregate(t[,c("ACT.COST","ITEMS")],by=list(t$BNF.CODE,t$PERIOD),FUN=sum)
    names(problem.spend)<-c("Drug","Period","Spend","Items")
    problem.spend$Spend<-round(problem.spend$Spend,digits=0)
    problem.spend$Drug<-as.character(problem.spend$Drug)
    total.problem.spend<-rbind(total.problem.spend,problem.spend)
    ## Calculations by practice
    s<-aggregate(t[,c("ACT.COST","ITEMS")],by=list(t$PRACTICE,t$PERIOD,t$BNF.CODE),FUN=sum)
    names(s)<-c("Practice.code","Month","Drug","cost.thisdrug","items.thisdrug")
    s<-merge(s,surgery.subtotal,all.x=TRUE)
    s<-merge(s,short.addresses,by.x="Practice.code",by.y="V2",all.x=TRUE)
    spend.practice<-rbind(spend.practice,s)
  ...
  ## Write out summary files for main analysis process
  write.csv(spend.practice,paste0("spend_practice.",filesep, ".csv"),row.names=FALSE,quote=FALSE)
  ## write.csv(problem.drugs,paste0("problem_drugs.",filesep, ".csv"),row.names=FALSE,quote=FALSE)
  write.csv(problem.spend,paste0("problem_spend.",filesep, ".csv"),row.names=FALSE,quote=FALSE)
  write.csv(total.problem.spend,paste0("total_problem_spend.",filesep, ".csv"),row.names=FALSE,quote=FALSE)
